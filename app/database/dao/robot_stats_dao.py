...
def update_robot_stats(
    ...
    is_tier: bool,
    is_loser: bool,
    games_win_proportion: int
):
    robot: Robot = get_bot_by_id(robot_id)
    robot_stats: RobotStats = robot.stats
    ...
    try: 
        robot.stats.set(
            ...
            matches_won=(current_matches_won+1) if is_winner else current_matches_won,
            matches_tied=(current_matches_tied+1) if is_tier else current_matches_tied,
            matches_lost=(current_matches_lost+1) if is_loser else current_matches_lost,
            games_win_rate=(current_games_win_rate+games_win_proportion)/2
        )
        return True
    except: 
        return False
