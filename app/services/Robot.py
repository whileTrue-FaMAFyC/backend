...
class Robot:
    ...
    def _scan(self, robots_positions):
        ...
        # 359, so we add 360 to get the equivalent angle in an appropiate
        # range.
        if max_angle > 359:
            max_angle -= 360
            new_max = True
    ...
    def _attack(self):
        if self.is_cannon_ready() and self._cannon_distance > 0:
            distance_x = round_up(
                round(cos(radians(self._cannon_direction)), 5) *
                self._cannon_distance
            )
            distance_y = round_up(
                round(sin(radians(self._cannon_direction)), 5) *
                self._cannon_distance
            )
